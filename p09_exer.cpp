...
GLfloat ambient1[4] = { 0.1f, 0.1f, 0.1f, 1.0f };
GLfloat diffuse1[4] = { 1.0f, 1.0f, 1.0f, 1.0f };
GLfloat specular1[4] = { 0.1f, 0.1f, 0.1f, 1.0f };
...
void init() {
	...
	cout << "keyboard Input: left for decreasing shininess" << endl;
	cout << "keyboard Input: right for increasing shininess" << endl;
	...
	cout << "keyboard Input: 4 for a plane with many small squares" << endl;
}
...
void setupColorMaterial(const vec3& color)
{
...
bool readMesh(const char* filename)
{
	...
	{
		is >> n >> face[0][i] >> face[1][i] >> face[2][i]; // i번째 삼각형의 0,1,2번 vertex
		...
		fnormal[i] = v; //fnormal[0] = 삼각형 한 면의 normal vector 

		//vnormal = 삼각형을 이루는 각 vertex들에 normal vector 만큼 더해준 것
		vnormal[face[0][i]] += v;
		vnormal[face[1][i]] += v;
		vnormal[face[2][i]] += v;
	}
	for (int i = 0; i < nVertices; i++)
	{
		vnormal[i] = normalize(vnormal[i]); // 단위벡터화
	}
...
{
	glLightfv(GL_LIGHT0 + i, GL_AMBIENT, ambient1);
	glLightfv(GL_LIGHT0 + i, GL_DIFFUSE, diffuse1);
	glLightfv(GL_LIGHT0 + i, GL_SPECULAR, specular1);
...
void drawSpotLight(const vec3& p, float cutoff)
{
	...
	setupColorMaterial(vec3(0, 0, 1));

	float h = 0.15f;
	float r = h * tan(radians(cutoff));
	drawCone(r, h, 16, 5);

	setupColorMaterial(vec3(1, 0, 0));
...
void keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		case GLFW_KEY_RIGHT: ambient1[0] += 0.1f; ambient1[1] += 0.1f; ambient1[2] += 0.1f; ambient1[3] += 1.0f;
			diffuse1[0] += 1.0f; diffuse1[1] += 1.0f; diffuse1[2] += 1.0f; diffuse1[3] += 1.0f;
			specular1[0] += 0.1f; specular1[1] += 0.1f; specular1[2] += 0.1f; specular1[0] += 1.0f; break;
		case GLFW_KEY_LEFT: ambient1[0] -= 0.1f; ambient1[1] -= 0.1f; ambient1[2] -= 0.1f; ambient1[3] -= 1.0f;
			diffuse1[0] -= 1.0f; diffuse1[1] -= 1.0f; diffuse1[2] -= 1.0f; diffuse1[3] -= 1.0f;
			specular1[0] -= 0.1f; specular1[1] -= 0.1f; specular1[2] -= 0.1f; specular1[0] -= 1.0f; break;
